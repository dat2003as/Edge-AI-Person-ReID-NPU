...
class FaceDetection:
    """ 
    Lớp phát hiện khuôn mặt sử dụng API MediaPipe Tasks mới nạp từ RAM để tránh lỗi đường dẫn Windows.
    """
    def __init__(self,
                 model_name: str = "blaze_face_short_range.tflite",
                 min_detection_confidence: float = 0.4):
        
        # 1. Xác định đường dẫn file model (vẫn cần để Python tự mở file)
        ...
        # --- 🔹 BƯỚC FIX CHÍNH: NẠP MODEL VÀO RAM 🔹 ---
        try:
            # Tự mở file bằng Python để tránh lỗi MediaPipe nối chuỗi site-packages
            ...
            # Khởi tạo detector từ dữ liệu trong RAM
            self.detector = vision.FaceDetector.create_from_options(options)
            
            self.face_aligner = FaceAligner()
            self.fps_avg = 0.0
            self.call_count = 0
            print(f"✅ FaceDetection (Buffer Mode) đã khởi tạo thành công.")
            print(f"FaceDetection (MediaPipe Tasks) đã khởi tạo thành công với model: {model_name}")

        except Exception as e:
            print(f"❌ Lỗi khi nạp Face Detector qua Buffer: {e}")
            raise
